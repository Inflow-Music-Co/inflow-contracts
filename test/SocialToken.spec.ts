...
describe("SocialToken Tests", async function () {
  ...
  //       await expect(socialMinter.mint(diff.add(10), usdc.address)).to.be
  //         .reverted;
  //     } catch (err) {
  //       console.error(err);
  //     }
  ...
  //       expect(await social.balanceOf(minterAddress)).to.equal(
  //         AMOUNT.add(AMOUNT)
  //       );
  //       expect(await usdt.balanceOf(minterAddress)).to.equal(0);
  //     } catch (err) {
  ...
  //       const burnPrice = await burn(socialMinter, AMOUNT);
  //       expect(await social.balanceOf(minterAddress)).to.equal(AMOUNT);
  //       expect(await usdc.balanceOf(minterAddress)).to.equal(
  //         _minter_bal.add(_usdc_bal)
  //       );
  //       expect(await usdt.balanceOf(minterAddress)).to.equal(
  //         _getburnPrice.sub(_usdc_bal)
  //       );
  //     } catch (err) {
  //       console.error(err);
  //     }
  ...
  //   it("17 - Should emit MintedData", async () => {
  //     try {
  //       expect(mint(socialMinter, usdcMinter, AMOUNT))
  //         .to.emit(social, "Minted")
  //         .withArgs(AMOUNT, usdc.address);
  //     } catch (err) {
  //       console.error(err);
  //     }
  //   });
  it("18 - Should pay both the artist and the admin fee", async () => {
    try {
        console.log("1");
        console.log({adminAddress, usdc})
        try {
            const adminPreBalance = await social.balanceOf(adminAddress);
        } catch (err) {
            console.log({err})
        }
      console.log("2");

      const artistPreBalance = await usdc.balanceOf(adminAddress);
      console.log("3");

      const minted = await mint(socialMinter, usdcMinter, 0.25);
      
      console.log({minted})
      const adminPostBalance = await usdc.balanceOf(adminAddress);
      console.log("4");

      const artistPostBalance = await usdc.balanceOf(adminAddress);
      console.log("5");

    //   expect(adminPostBalance).to.be.above(adminPreBalance);
      console.log("6");

      expect(artistPostBalance).to.be.above(artistPreBalance);
      console.log("7");
